...
Student::Student(const Student &stu){
    ...
    nCoursesTaken = stu.nCoursesTaken;
    for(int k = 0; k<nCoursesTaken; k++){
        coursesTaken[k] = stu.coursesTaken[k];
    }
}
...
Student & Student::operator=(const Student &stu){
    ...
    if(this == &stu)
        ...
        delete []coursesTaken;
        studentID = stu.studentID;
        for(nCoursesTaken=0;nCoursesTaken<stu.nCoursesTaken;nCoursesTaken++){
            coursesTaken[nCoursesTaken] = stu.coursesTaken[nCoursesTaken];
        }
    }
    return *this;
}
